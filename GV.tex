...
%%%%%%%%%%%%%%%%%%%%
\definecolor{titlecol}{rgb}{1,0,0}
...
Since the blue cloud is considered to be primarily made of star forming galaxies we expect the model to have some difficulty in determining the most likely quenching model to describe them, as seen in Figure \ref{blue_c}. In Figure \ref{blue_c} (at this point we remind the reader that although a galaxy has undergone quenching, star formation can still be occurring in a galaxy, just at a slower rate than at an earlier time). 

Of all of the regions of the CMD, this is to be expected in the blue cloud since it consists of galaxies that are predominantly still star forming, confirmed by the inability of the model to attribute the extremely blue colours of the majority of these galaxies to fast quenching at recent times (i.e. very little change in the SFR; see the right hand panel of Figure \ref{blue_c} in particular). The way these galaxies must therefore differ, is in the mechanisms and triggers by which they have formed their stars, having no effect on the observed colours, but a discernible effect on their morphologies.

{\changed We do see some preference for slow quenching of blue bulge-dominated galaxies which began at $z \sim 1 $, therefore these populations must have been blue for a considerable period of time, slowly using up their gas for star formation by the Kennicutt–Schmidt law \citep{Schmidt59, Kennicutt97}. However we also see some preference for rapid quenching at recent times in the blue cloud; again it is remarkable that this has been detected due to the speed at which such a quenching timescale would move a galaxy across the colour-magnitude diagram. This therefore supports the theories for blue ellipticals as either merger-driven ($\sim23\%$) or gas inflow driven reinvigorated star formation that is now slowly decreasing ($\sim77\%$).}

The blue cloud is therefore primarily composed of both star forming disc and smooth galaxies and smooth galaxies which have been quenched with slow to intermediate timescales slowly moving across the colour-magnitude diagram.
...
\item {\changed The green valley as we currently observe it is dominated by very slowly evolving disc-like galaxies along with intermediate- and smooth-like galaxies which pass across it with intermediate timescales within $\sim 1.0-1.5~\rm{Gyr}$ (see Section \ref{gv}).}
...
\item {\changed Blue cloud galaxies cannot be well fit by a quenching model of star formation due to the continuous high star formation rates occurring (see Figure \ref{blue_c}).}

\item {\changed Disc galaxies preferences for quenching time are distributed evenly over all redshifts; confirming the assumption that whatever is driving the evolution of discs is constant over cosmic time, unlike for elliptical galaxies which has been observed to be driven by downsizing.}

\item {\changed Rapid quenching timescales are detected with a rarer occurrence for green valley galaxies than red sequence galaxies.} We attribute this quenching mechanism to major mergers with black hole feedback which are able to expel the remaining gas not initially exhausted in the merger induced starburst and often causes a change in morphology from disc dominated to bulge dominated. The colour-change timescales from previous simulations of such events agree with our derived rapid timescales  {\changed (see Section \ref{rapid}). These rapid timescales were key in forming the red sequence, however galaxies currently passing through the green valley which will eventually build up on the red sequence do so at more intermediate timescales (see Figure \ref{green_v}).}
