...
We assume that all galaxies formed at a time $t=0~\rm{Gyr}$ with an initial burst of star formation. The mass of this initial burst is controlled by the value of the $i_{sfr}$ which is set as the average specific SFR (sSFR) at the time of quenching $t_q$. By empirically fitting to SDSS data, \citet{Peng} defined a relation (their equation 1) for the average sSFR and redshift (cosmic time, $t$) as:
\begin{equation}
...
{\changed The disc galaxy parameters for the green valley population now overwhelmingly prefer slow quenching timescales ($47.4\%$) with a similar amount of intermediate quenching compared to the smooth galaxy parameters ($37.6\%$; see Figure~\ref{green_v}).} Compared to red sequence disc galaxies, there is still some preference for galaxies with a star formation history which results in a high current SFR, suggesting there are also some late-type galaxies that have just progressed from the blue cloud into the green valley. 
...
Inspection of the galaxies contributing to this area of probability reveals that this does not arise due to \emph{currently} merging pairs identified by GZ users, but by typical smooth galaxies with red optical and NUV colours that the model attributes to rapid quenching at early times.
...
\begin{enumerate}
\item Red sequence galaxies, regardless of morphology, are found to have similar preferences for quenching timescales compared to the green valley galaxies, but occurring at earlier quenching times (see Figures~\ref{red_s} and~\ref{green_v}). Therefore the quenching mechanisms currently occurring in the green valley were active in creating the red sequence at earlier times; confirming that the green valley is indeed a transitional population, regardless of morphology.

\item {\changed The typical red sequence galaxy is elliptical in morphology and has undergone a rapid to intermediate quench at any point in cosmic time, resulting in a very low current SFR (see Section~\ref{rs}).}
